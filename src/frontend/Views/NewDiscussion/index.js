...
class NewDiscussion extends Component {
  ...
  handleChangeFile = (file) => {
    ...
    async function storePollItem(polltitle,item){
      console.log('jsm');
      const accounts = await cav.klay.getAccounts();
      const acc = accounts[0];
      cav.klay.personal.unlockAccount(acc,'0000',30000);
      await pollcontract.methods.storeItem(polltitle,item).send({from: acc,gas : 25000000});
    ...
    reader.onload = (file2) => {
        ...
        poll_items.languages.map((data,i)=>{
          var que='';
          que += data.name;
          data.vote.map((ans,i)=>{
            que += '#'+ans;
        ...
        storePollItem(this.state.votetitle,poll);
        console.log(poll);
    ...
    if (authenticated) {
      return [
        <input
          ...
          value={title}
          onChange={(event) => { updateDiscussionTitle(event.target.value);this.updateTitle(event.target.value); }}
        />,
        ...
        />,
        (role === 'admin' && this.state.votetitle.length >= 15 && pinned) && <input type='file' key={'poll'} id="regpoll_items" onChange={ e=>
          this.handleChangeFile(e.target.files[0])}></input>,
        <TagsInput
          key={'tags'}
          value={tags}
